...
We need to make sure that the chosen techonology stack can survive the vendors that proposed it and that is flexible enough to be able deviate from the original intended standard if and when needed.

In order to do that we need to establish a governing body that will overlook the process of definition and selection of the standard and will manage the economical and technical investment.

Having a so-called Vendor Neutral Architecture (VNA) provides an example of this. These concepts feed into to the definition of _enterprise architecture_. 
...
2. A _Solution Architecture_ describes in detail the actual business operations, activities and processes and how IT systems support them. It typically applies to a single project/organisation.

Agilists may comment that architeture it's a bit of a waterfall exercise (BDUF), and might not be neeeded at all (YAGNI). I don't agree with that, the work of [Meyer] and [Babar et al.] describe the reasons why.
...
At this point I think we made the case for interoperability as one of the core requirements to develop and choose software for healthcare.

Now we need to think about how to achieve interoperability. 

So we need a standard, and maybe we need a _organisation_ on top of that which supports and governs it, then we need a testing infrastucture, then certification comes to mind, quickly the task might easily become daunting and hard to tackle. Why repeating this process multiple times rather than do it once and all agree on one solution worldwide. 

Is that a good idea? I don't think so. I want to tell you the story of the European Interoperability Framework (EIF) and the Free Software Foundation to try and convince you.

At the beginning of the century, the EU commission made the right decision: to make interoperability mandatory amongst public services in Europe.

A hypothetical IT employee within the EU would have said, "We know that a standard is not sufficient, so we bring in a Enterprise Architecture framework.".

EIF quickly evolved to version 2.0 which raised a substantial political debate due to political lobbying from the Business Software Association (BSA). The Free Software Foundation produced a [statement](https://fsfe.org/activities/os/eifv2.en.html) as follows:
...
The basic idea of the Business Software Alliance lobbying was that software _homogeneity_ guarantees interoperability, while other approaches, well, probably not. BSA clearly wanted to achieve a vendor lock-in situation, disqualifying the continuum concept described above.

The EIFv3 (or the New EIF) explicitly promotes open standards, and [Free Software has been indicated as one of the factors of enhanced interoperability](https://fsfe.org/activities/os/eif-v3.en.html).


MA NON MANCA UNA FRASINA DI CONCLUSIONE PER RIALLACCIARSI AL DISCORSO? NON SCRIVI PERCHE' LA BFA HA PERSO?

# Part 2

## Blockchain and Interoperability

Blockchain is a distributed ledger, and is, in this text from now on referred to as a Distributed Ledger Technology (DLT). [Wattenhofer] defines the blockchain _as the longest path from the genesis block, i.e., root of the tree, to a leaf._

_The blockchain acts as a consistent transaction history on which all nodes eventually agree._

On each transaction some code can be executed in different ways and takes different names (e.g., Smart Contracts, Chaincode, etc). From this definition it should be clear that **the blockchain is not an efficient database**. Storing unstructured data on a blockchain guarantees poor performance but still, depends on the blockchain deployment, whether public or private and on the consensus algorithm. [Here](https://blockchainatberkeley.blog/building-it-better-a-simple-guide-to- blockchain-use-cases-de494a8f5b60) and [here](https://eprint.iacr.org/2017/375.pdf) there are pretty famous blockchain papers on proper DLTs use.
